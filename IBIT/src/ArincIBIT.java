...
public class ArincIBIT {
	...
	private static ArrayList<String> newTestSites=new ArrayList<String>();
	private static ArrayList<Boolean> whattoTestFor=new ArrayList<Boolean>();
			...
			if(i<errorSites.size()&&count<=(errorSites.size()/3)){
				...
				if(errorSites.size()>3){
					if(errorSites.get(i+2).equals("duel")){
						duelSite=true;
						Site2=errorSites.get(i+3);
						IpAddress3=errorSites.get(i+4);
						Port2=Integer.parseInt(errorSites.get(i+5));
						IpAddress2=errorSites.get(i+6);
						Port2=Integer.parseInt(errorSites.get(i+7));
	...
	public static void main(String[] arg) throws Exception {
		Arinc newArinc=new Arinc();
		Scanner fileScanner = null;
		while(!newArinc.getRun()){
			newArinc.setVisible(true);
		}newArinc.setVisible(false);
		String desktopPath="";
		try{
		...
		FileStructure newFileStructure=new FileStructure(desktopPath+"\\IBIT","emailList",".txt");
		...
		newTestSites=newArinc.getTestSites();
		if(newArinc.getIBIT()&&newArinc.getReset()&&newArinc.getHealth()){
			whattoTestFor.add(newArinc.getFWR());whattoTestFor.add(newArinc.getREF());
			whattoTestFor.add(newArinc.getSWR());whattoTestFor.add(newArinc.getMod());
			whattoTestFor.add(newArinc.getSens());whattoTestFor.add(newArinc.getBool1());
			whattoTestFor.add(newArinc.getBool2());
			setFiles();
			int i=0;
			while(i<newTestSites.size()){
				Site=newTestSites.get(i);
				IpAddress=newTestSites.get(i+1);
				Port=Integer.parseInt(newTestSites.get(i+2));
				if(newTestSites.get(i+3).toLowerCase().equals("end")){
					i=i+4;
					reset();
					ibit();
					health();
					...
					duelSite=true;
					Site2=newTestSites.get(i+3);
					IpAddress3=(newTestSites.get(i+4));
					Port3=Integer.parseInt(newTestSites.get(i+5));
					IpAddress2=newTestSites.get(i+6);
					Port2=Integer.parseInt(newTestSites.get(i+7));
					i=i+9;
					reset();
					ibit();
					health();
		...
		}else if(!newArinc.getIBIT()&&newArinc.getReset()&&newArinc.getHealth()){
			reset();
			health();
